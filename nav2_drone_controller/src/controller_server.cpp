...
/* **********************************************************************
 * Action Server responding to nav2_drone_msgs/action/FollowPath
...
ControllerServer::ControllerServer(const rclcpp::NodeOptions & options)
  ...
  default_progress_checker_type_("nav2_drone_controller::SimpleProgressChecker"),
  goal_checker_loader_("nav2_drone_core", "nav2_drone_core::GoalChecker"),
  default_goal_checker_ids_{"SimpleGoalChecker"},
  default_goal_checker_types_{"nav2_drone_controller::SimpleGoalChecker"},
...
void ControllerServer::init()
{
 ...
 //     return nav2_drone_util::CallbackReturn::FAILURE;
  }

  // Set up goal checkers
  for (size_t i = 0; i != goal_checker_ids_.size(); i++) {
    try {
      nav2_drone_core::GoalChecker::Ptr goal_checker =
...
//        return nav2_drone_util::CallbackReturn::FAILURE;
  ...
  for (size_t i = 0; i != controller_ids_.size(); i++) {
    try {
      nav2_drone_core::Controller::Ptr controller =
...
void ControllerServer::execute(const std::shared_ptr<GoalHandleFollowPath> goal_handle)
{
  ...
  try
  {
    ...
    if (findControllerId(c_name, current_controller)) {
      ...
      throw nav2_drone_core::InvalidController("Failed to find controller name: " + c_name);
    }
    ...
    if (findGoalCheckerId(gc_name, current_goal_checker)) {
      ...
      throw nav2_drone_core::ControllerException("Failed to find goal checker name: " + gc_name);
    }
    ...
    while ( rclcpp::ok() ) {
      ...
      if (!getRobotPose(pose)) {
        throw nav2_drone_core::ControllerTFError("Failed to obtain robot pose.");
      }

      if (!progress_checker_->check(pose)) {
        throw nav2_drone_core::FailedToMakeProgress("Failed to make progress");
      }
      ...
      try {
        ...
        size_t current_idx = nav2_drone_util::find_closest_goal_idx( pose, goal->path);
        distance_to_goal = nav2_drone_util::calculate_path_length( goal->path, current_idx );
        goal_handle->publish_feedback(feedback);
      } catch (nav2_drone_core::DroneException & e) {
        RCLCPP_WARN(this->get_logger(), e.what());
      } catch (nav2_drone_core::ControllerTFError & e) {
        ...
        server_mutex.unlock();
        return;
      } catch (nav2_drone_core::NoValidControl & e) {
        RCLCPP_ERROR(this->get_logger(), "%s", e.what());
        publishZeroVelocity();
        ...
        server_mutex.unlock();
        return;
      } catch (nav2_drone_core::FailedToMakeProgress & e) {
        RCLCPP_ERROR(this->get_logger(), "%s", e.what());
        publishZeroVelocity();
        ...
        server_mutex.unlock();
        return;
      } catch (nav2_drone_core::PatienceExceeded & e) {
        RCLCPP_ERROR(this->get_logger(), "%s", e.what());
        publishZeroVelocity();
        ...
        server_mutex.unlock();
        return;
      } catch (nav2_drone_core::InvalidPath & e) {
        RCLCPP_ERROR(this->get_logger(), "%s", e.what());
        publishZeroVelocity();
        ...
        server_mutex.unlock();
        return;
      } catch (nav2_drone_core::ControllerException & e) {
        RCLCPP_ERROR(this->get_logger(), "%s", e.what());
        publishZeroVelocity();
  ...
  catch(nav2_drone_core::DroneException& ex)
  {
...
RCLCPP_COMPONENTS_REGISTER_NODE(nav2_drone_controller::ControllerServer)
