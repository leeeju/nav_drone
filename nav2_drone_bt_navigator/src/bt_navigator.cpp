...
: nav2_drone_util::LifecycleNode("bt_navigator", "",
...
nav2_drone_util::CallbackReturn
BtNavigator::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  plugin_lib_names = nav2_drone_util::split(nav2::details::BT_BUILTIN_PLUGINS, ';');
  ...
  odom_smoother_ = std::make_shared<nav2_drone_util::OdomSmoother>(node, filter_duration_, odom_topic_);
  ...
  const std::vector<std::string> default_navigator_types = {
    "nav2_drone_bt_navigator::NavigateToPoseNavigator",
    "nav2_drone_bt_navigator::NavigateThroughPosesNavigator"
  };
  ...
  for (size_t i = 0; i != navigator_ids.size(); i++) {
    try {
      std::string navigator_type = nav2_drone_util::get_plugin_type_param(node, navigator_ids[i]);
      RCLCPP_INFO(
  ...
  return nav2_drone_util::CallbackReturn::SUCCESS;
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(nav2_drone_bt_navigator::BtNavigator)
