...
#include "nav2_drone_mpc_controller/mpc_controller.hpp"
      ...
      const geometry_msgs::msg::Twist & speed)
{
  ...
  //nav2_drone_util::transformPoseInTargetFrame(goal_pose, carrot_pose, *tf_, "base_link" );
...
       sphere size.
  */
  ...
  double target_elevation = nav2_drone_util::rad_to_deg( target_ez.first );
  double target_azimuth = nav2_drone_util::rad_to_deg( target_ez.second );
  ...
  point.pose.position.x = bounding_box_radius * cos(nav2_drone_util::deg_to_rad(azimuth)) + current_pose.pose.position.x;
  point.pose.position.y = bounding_box_radius * sin(nav2_drone_util::deg_to_rad(azimuth)) + current_pose.pose.position.y;
  point.pose.position.z = bounding_box_radius * sin(nav2_drone_util::deg_to_rad(plunge)) + current_pose.pose.position.z;
...
// Register this controller as a nav2_drone_core plugin
PLUGINLIB_EXPORT_CLASS(
  nav2_drone_mpc_controller::MPCController,
  nav2_drone_core::Controller)
